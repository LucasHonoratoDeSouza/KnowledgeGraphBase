...
- Make direct manipulation feel light: a dragged node follows the pointer while
  connected nodes react and settle with damped motion.
...
1. WHEN a node is dragged THEN it SHALL follow the graph-space pointer position.
...
3. WHEN the node is released THEN it SHALL be unpinned and the simulation SHALL
   decay to rest while preserving collision separation and stage bounds.
4. WHEN graph input includes a missing endpoint THEN the simulation SHALL ignore
   that edge without producing non-finite coordinates.
...
2. WHEN Zoom in or Zoom out is activated THEN zoom SHALL change by one `1.25x`
   step around the viewport center.
...
3. WHEN the simulation reaches its rest threshold THEN it SHALL stop requesting
   animation frames until an interaction reheats it.

### FG-05 — Existing states and accessibility (P2)

1. WHEN reduced motion is requested THEN the graph SHALL paint a settled layout
   without a visible animated sequence.
...
- Pure unit tests for force response, decay, collision/bounds, invalid edges and
  camera anchoring/clamps.
- Component tests for named controls, viewport state and drag cleanup.
